...
#let iconlink(
    ...
    let icon-body = [ #if size != none [#text(size, icon)] else [#icon] ]
    let label-body = [ #if size != none [#text(size, label)] else [#label] ]
    let content = [ #fa(icon-body) #if label != none [#label-body] ]
    link(uri)[
        #box(
